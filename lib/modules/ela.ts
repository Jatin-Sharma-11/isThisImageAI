...
export const analyzeELA = async (img: HTMLImageElement): Promise<ELAResult> => {
    ...
    // Widened thresholds significantly
    if (mean < 15) {
        score += 0.8; // Very strong indicator of synthetic generation
    } else if (mean < 30) {
        score += 0.5;
    }

    // Check for Uniformity (Low Variance relative to mean)
    // Coefficient of variation might be useful, but raw variance is okay for now.
    ...
    if (variance < 200) {
        score += 0.5; // Very uniform
    } else if (variance > 800) {
        score -= 0.2; // High variance (good sign of real manipulation/compression)
    }
